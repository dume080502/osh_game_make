...
class Player:
    ...
    def draw(self, surface):
        ...
        arm_offset = math.sin(self.walk_count + math.pi) * 8 # 팔은 반대로 흔들리게
        
        # 중심점 계산
        head_center_x = int(self.x + 20)
        head_center_y = int(self.y + 10)
        ...
        # 4. 머리
        pygame.draw.circle(surface, SKIN, (head_center_x, head_center_y), 9)

        # 5. 눈 추가 (위쪽을 볼 때는 안 그려서 뒷모습 표현)
        if self.facing_y >= 0: # 아래나 옆을 볼 때
            eye_x_shift = self.facing_x * 4
            eye_y_shift = self.facing_y * 2
            # 왼쪽 눈
            pygame.draw.circle(surface, EYE_COLOR, (head_center_x - 3 + eye_x_shift, head_center_y + eye_y_shift), 2)
            # 오른쪽 눈
            pygame.draw.circle(surface, EYE_COLOR, (head_center_x + 3 + eye_x_shift, head_center_y + eye_y_shift), 2)

        # 6. 모자 추가 (방향에 따라 챙 위치 조절)
        hat_brim_x = head_center_x - 12 + (self.facing_x * 3)
        # 모자 몸체
        pygame.draw.rect(surface, HAT_COLOR, (head_center_x - 8, head_center_y - 15, 16, 9))
        # 모자 챙
        pygame.draw.rect(surface, HAT_COLOR, (hat_brim_x, head_center_y - 8, 24, 4))
